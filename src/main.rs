...
fn main() -> ! {
    ...
    let mut matrix: Matrix<6, 5> = Matrix::new(
        [
            gpio0.p0_26.into_push_pull_output(Level::Low).degrade(),
            gpio0.p0_31.into_push_pull_output(Level::Low).degrade(),
            gpio0.p0_29.into_push_pull_output(Level::Low).degrade(),
            gpio0.p0_02.into_push_pull_output(Level::Low).degrade(),
            gpio1.p1_15.into_push_pull_output(Level::Low).degrade(),
            gpio1.p1_13.into_push_pull_output(Level::Low).degrade(),
        ],
        [
            gpio0.p0_10.into_floating_input().degrade(),
            gpio0.p0_09.into_floating_input().degrade(),
            gpio0.p0_24.into_floating_input().degrade(),
            gpio0.p0_22.into_floating_input().degrade(),
            gpio0.p0_20.into_floating_input().degrade(),
        ],
    );
    ...
    loop {
        ...
        if matrix.state.flatten().contains(&matrix::KeyState::Down) {
            // unwrap: infallible
        ...
        let events = mapper.step(&matrix.state);
